...
def discover_detection_jsons(detections_dir: Path) -> list[Path]:
    return sorted(
        ...
        if path.is_file() and path.suffix.lower() == SUPPORTED_JSON_SUFFIX
    )
...
def export_from_detection_json(
    json_path: Path,
    crops_dir: Path,
    *,
    padding: int,
    ...
    for item in boxes:
        ...
        image_rel_path = Path("dataset") / "crops" / f"{sample_id}.png"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export cropped letter-block images from detection JSON files.")
    parser.add_argument("--detections-dir", required=True, help="Directory containing per-image detection JSON files.")
    parser.add_argument("--crops-dir", required=True, help="Directory to write cropped letter images.")
    parser.add_argument("--labels-json", required=True, help="Path to write the labels manifest JSON.")
    parser.add_argument("--padding", type=int, default=DEFAULT_PADDING, help="Extra pixels around each bbox crop.")
...
def main() -> None:
    ...
    for json_path in detection_jsons:
        records, next_index = export_from_detection_json(
            json_path,
            crops_dir,
